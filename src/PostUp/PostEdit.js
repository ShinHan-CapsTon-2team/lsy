...
function PostEdit() {
  ...
  const classifyImage = async (img, threshold) => {
    try {
      ...
      const softmaxPrediction = tf
        .softmax(tf.tensor(prediction))
        .arraySync()[0];
...
const handleImageFileChange = async (event, index) => {
  ...
  if (
    ...
    setUpdatedPost(
      (
        prevPosts // 이전에 저장한 이미지의 post 객체를 업데이트
      ) =>
        prevPosts.map((post, i) =>
          i === index
            ? {
  ...
  const handleCategorySelect = (selectedCategory, index) => {
    console.log(
      `handleCategorySelect - index: ${index}, selectedCategory: ${selectedCategory}`
    );
    setUpdatedPost((prevPosts) =>
      prevPosts.map((post, i) =>
...
const handleUpdate = (index) => {
  if (updatedPost[index] && updatedPost[index].newImageFile) {
    ...
    const englishCategory =
      categoryLabel[updatedData.category] || updatedData.category;
    formData.append(
      "data",
      JSON.stringify({ ...updatedData, category: englishCategory })
    );
                    ...
                    updatedPost.map((post, index) => {
                      ...
                      return (
                        <div key={index}>
                          ...
                          <S.Three>
                            <S.SelectImg
                              src={post.imagePreviewUrl || imageUrl}
                              alt={`게시글 이미지`}
                            />
                            <S.FindImg>
                              <S.Menu
                            ...
                            <S.FileBox
                              ...
                              onChange={(e) =>
                                handleImageFileChange(e, index, "image_url")
                              }
                            />
                ...
                <S.Buttons>
                        ...
                        <S.DropContainer>
                          {isMenuOpen && (
                            <S.DropMenu>
                              ...
                              <S.CateMenu
                                onClick={() =>
                                  handleCategorySelect(classLabels[0], index)
                                }
                              >
                                {classLabels[0]}
                              </S.CateMenu>
                              <S.CateMenu
                                onClick={() =>
                                  handleCategorySelect(classLabels[1], index)
                                }
                              >
                                {classLabels[1]}
                              </S.CateMenu>
                              <S.CateMenu
                                onClick={() =>
                                  handleCategorySelect(classLabels[2], index)
                                }
                              >
                                {classLabels[2]}
                              </S.CateMenu>
                              <S.CateMenu
                                onClick={() =>
                                  handleCategorySelect(classLabels[3], index)
                                }
                              >
                                {classLabels[3]}
                              </S.CateMenu>
                              <S.CateMenu
                                onClick={() =>
                                  handleCategorySelect(classLabels[4], index)
                                }
                              >
                                {classLabels[4]}
                              </S.CateMenu>
                              <S.CateMenu
                                onClick={() =>
                                  handleCategorySelect(classLabels[5], index)
                                }
                              >
                                {classLabels[5]}
                              </S.CateMenu>
                  ...
                  <S.Right>
                    {updatedPost.map((post, index) => (
                      <div key={post.id}>
                        {/* 게시물 내용 렌더링 */}
                        <S.EditButton onClick={() => handleUpdate(index)}>
                          업로드
                        </S.EditButton>
                      </div>
                    ))}
